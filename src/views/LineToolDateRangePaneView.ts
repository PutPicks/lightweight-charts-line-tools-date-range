...
export class LineToolDateRangePaneView<HorzScaleItem> extends LineToolPaneView<HorzScaleItem> {
	...
	private _addDateDifferenceLabel(
		...
		const logicalIndex0 = timeScale.timeToCoordinate(time0Raw.timestamp as any);
		const logicalIndex1 = timeScale.timeToCoordinate(time1Raw.timestamp as any);
		...
		if (barCount < 1 && calendarDays > 0) barCount = 1;
